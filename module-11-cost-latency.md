# Module 11: Cost, Latency & Model Selection
...
```mermaid
flowchart TD
    Start["Does the task need:"] --> RT{"Real-time\ninternet access?"}
    RT -- Yes --> WS["Add web search tool\n(any model)"]
    RT -- No --> YD{"Access to\nyour data?"}
    YD -- Yes --> RAG["Add RAG\n(any model)"]
    YD -- No --> CR{"Complex reasoning\n+ high stakes?"}
    CR -- Yes --> Opus["Opus"]
    CR -- No --> HV{"High volume\n+ simple task?"}
    HV -- Yes --> HaikuTest["Test Haiku first"]
    HaikuTest --> QOK{"Quality\nsufficient?"}
    QOK -- Yes --> Haiku["Haiku"]
