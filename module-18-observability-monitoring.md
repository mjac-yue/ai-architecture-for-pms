...
flowchart TD
    ...
    Q1 -- "No" --> P1["Check temperature settings\nLog full trace next time"]
    Q1 -- "Yes" --> Q2{"Where in trace did\nit go wrong?"}
    Q2 -- "Retrieval" --> R1["Wrong chunks retrieved\n→ Improve indexing or query"]
    Q2 -- "Model call" --> R2["Model produced bad output\n→ Prompt issue or model regression"]
    Q2 -- "Post-processing" --> R3["Output parsing or formatting\n→ Code bug, easier to fix"]
    Q2 -- "Tool use" --> R4["Wrong tool called or bad params\n→ Tool description or routing logic"]
```
