...
```mermaid
flowchart TD
    O["Orchestrator (routing logic)"]
    A1["Agent A"]
    A2["Agent B"]
    A3["Agent C"]
    S["Shared state (what gets passed between agents)"]
...
```mermaid
flowchart LR
    I["Input"] --> A1["Agent 1 Researcher"] --> A2["Agent 2 Writer"] --> A3["Agent 3 Editor"] --> O["Output"]
```
...
```mermaid
flowchart LR
    A["Agent 1 produces draft"] --> C["Agent 2 critiques"]
    C --> A2["Agent 1 revises"]
    A2 --> O["Final output"]
```
...
```mermaid
flowchart TD
    M["Master Agent overall goal"]
    M1["Manager A sub-domain 1"]
    M2["Manager B sub-domain 2"]
    W1["Worker"]
    W2["Worker"]
    W3["Worker"]
    W4["Worker"]
