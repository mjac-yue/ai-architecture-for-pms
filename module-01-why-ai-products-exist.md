...
flowchart LR
    subgraph Before["Before LLMs"]
        D1["Deterministic tasks\n(rules, math, retrieval)"] --> S1["Software"]
        L1["Language / judgement tasks\n(read, write, synthesise)"] --> H1["Humans"]
    end
    subgraph After["After LLMs"]
        D2["Deterministic tasks"] --> S2["Software"]
        L2["Language / judgement tasks"] --> AI["AI + Humans\n(now both viable)"]
    end
```
...
graph LR
    AI["AI Product"]
    AI --> P1["Effort Reduction\nFaster / cheaper than humans"]
    AI --> P2["Capability Extension\nDoes what humans can't"]
    AI --> P3["Decision Improvement\nMakes humans more accurate"]
```
...
```mermaid
flowchart TD
    C["Compute scale\n(GPUs, training infra)"] --> AI["Production-viable AI"]
    D["Data availability\n(internet-scale text)"] --> AI
    A["Architecture\n(transformer, scaling laws)"] --> AI
    AI --> E["Cost economics\n(API access at <$0.01/request)"]
    E --> P["Product viability\nat scale"]
```
...
```mermaid
flowchart TD
    A["Feature idea"] --> Q1{"Is the task\nunstructured / generative\n/ judgement-based?"}
    Q1 -- No --> X1["Deterministic system\nis the right answer"]
    Q1 -- Yes --> Q2{"Does it map to\nat least one value pattern?\n(effort / capability / decision)"}
    Q2 -- No --> X2["Value isn't sharp enough\nrefine the problem first"]
    Q2 -- Yes --> Q3{"Is occasional error\ntolerable here?"}
    Q3 -- No --> X3["Need a human-review layer\nor pick a different problem"]
    Q3 -- Yes --> Q4{"Would users actually\nvalue the outcome?"}
    Q4 -- No --> X4["Solving wrong problem\nfor this user"]
    Q4 -- Yes --> Y["AI is likely the right tool\n→ continue to feasibility spike"]
```
